...
namespace WPM {
    ...
    public class WorldMapEditorInspector : Editor {
        ...
        const string INFO_MSG_CATEGORY_DELETED = "Category deleted!";
        ...
        GUIContent[] reshapeRegionToolbarIcons, reshapeCityToolbarIcons, reshapeMountPointToolbarIcons, reshapeCategoryToolbarIcons, createToolbarIcons;
        int[] controlIds;
        bool startedReshapeRegion, startedReshapeCity, startedReshapeMountPoint, startedReshapeCategory, undoPushStarted;
        long tickStart;
        string[] reshapeRegionModeExplanation, reshapeCityModeExplanation, reshapeMountPointModeExplanation, reshapeCategoryModeExplanation, editingModeOptions, editingCountryFileOptions, createModeExplanation, cityClassOptions, categoryClassOptions, languageOptions;
        int[] cityClassValues;
        int[] categoryClassValues;
        int[] languageOptionsValues;
        
        string[] emptyStringArray;
        EditorAttribGroup mountPointAttribGroup, countryAttribGroup, provinceAttribGroup, cityAttribGroup, categoryAttribGroup;
        bool zoomed;
        ...
        void OnEnable() {
            ...
            createToolbarIcons[(int)CREATE_TOOL.CATEGORY] = new GUIContent("Event", icons[19], "Create a new Event");
            
            createModeExplanation = new string[CREATE_TOOLS_COUNT];
            createModeExplanation[(int)CREATE_TOOL.CITY] = "Click over the map in SceneView\nto create a NEW CITY for current COUNTRY";
            ...
            createModeExplanation[(int)CREATE_TOOL.CATEGORY] = "Click over the map in SceneView\nto create a NEW EVENT for current COUNTRY";
            ...
            // Setup reshape category tools
            int RESHAPE_CATEGORY_TOOLS_COUNT = 2;
            reshapeCategoryToolbarIcons = new GUIContent[RESHAPE_CATEGORY_TOOLS_COUNT];
            reshapeCategoryToolbarIcons[(int)RESHAPE_CATEGORY_TOOL.MOVE] = new GUIContent("Move", icons[18], "Move category");
            reshapeCategoryToolbarIcons[(int)RESHAPE_CATEGORY_TOOL.DELETE] = new GUIContent("Delete", icons[10], "Delete category");
            reshapeCategoryModeExplanation = new string[RESHAPE_CATEGORY_TOOLS_COUNT];
            reshapeCategoryModeExplanation[(int)RESHAPE_CATEGORY_TOOL.MOVE] = "Click and drag currently selected CATEGORY to change its POSITION";
            reshapeCategoryModeExplanation[(int)RESHAPE_CATEGORY_TOOL.DELETE] = "DELETES currently selected CATEGORY.";
            ...
            categoryClassOptions = new string[] {
                "ALL",
                "BATTLE",
                "ART",
                "DISCOVERY",
        ...
        public override void OnInspectorGUI() {
            ...
            */
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Label(" Show Only : ", GUILayout.Width(90));
            _editor.map.FilterCategoriesClass = EditorGUILayout.IntPopup(_editor.GUIFilterCategoryClass, categoryClassOptions, categoryClassValues);
            _editor.GUIFilterCategoryClass = _editor.map.FilterCategoriesClass;
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndHorizontal();
            ...
            /*
            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Show Categories", GUILayout.Width(90));
            bool prevShowCategories = _editor.map.showCategories;
            _editor.map.showCategories = EditorGUILayout.Toggle(_editor.map.showCategories, GUILayout.Width(20));
            
            
            //DROPDOWN Category FILTER x Category Class
            EditorGUILayout.BeginHorizontal();
            GUILayout.Label(" Show Only : ", GUILayout.Width(90));
            _editor.map.FilterCategoriesClass = EditorGUILayout.IntPopup(_editor.GUIFilterCategoryClass, categoryClassOptions, categoryClassValues);
            _editor.GUIFilterCategoryClass = _editor.map.FilterCategoriesClass;
            ...
            */
            ...
            EditorGUILayout.BeginVertical();
            /*
            //FILTER YEAR Category - 2 Slider
            EditorGUILayout.BeginHorizontal();
            
            EditorGUILayout.LabelField("Min Year Filter");
            
           // _editor.map.FilterCategoriesMinYear = EditorGUILayout.Slider("Start Year", _editor.GUIFilterCategoryMinYear, 0, 3000);
            EditorGUILayout.EndHorizontal();
            ...
            */
            ...
            switch (_editor.operationMode) {
                ...
                case OPERATION_MODE.CONFIRM:
                    ...
                    if (_editor.categoryChanges) {
                        DrawCenteredLabel("There're pending category modifications.");
                    }
                    EditorGUILayout.Separator();
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                ...
                case OPERATION_MODE.RESHAPE:
                    ...
                    if (_editor.categoryIndex >= 0) {
                        ShowReshapingCategoryTools();
                    }
                    break;
        ...
        void ShowEntitySelectors() {
            ...
            string[] cityNames = _editor.cityNames, mountPointNames = _editor.mountPointNames, categoryNames= _editor.categoryNames; /// DA MODIFICARE???
            ...
            if (_editor.countryIndex >= 0) {
                ...
                EditorGUILayout.BeginHorizontal();
                /*
                GUILayout.Label(" Domain ", GUILayout.Width(90));
                //Debug.Log("countryToDomain = " + selection);
                int domainSelection = EditorGUILayout.Popup(_editor.GUICountryIndex, countryNames);
                int countryToDomain = selection;
                if (domainSelection != countryToDomain)
                {
                    _editor.CountrySelectByDomain(domainSelection);
                }
                _editor.GUICountryNewDomain = countryNames[domainSelection];
                _editor.GUIDomainIndex = countryToDomain;
                if (GUILayout.Button("Update"))
                {
                    _editor.CountryNewDomain();
                }
                */
                
                GUILayout.Label("   Domain", GUILayout.Width(90));
                _editor.GUICountryNewDomain = EditorGUILayout.TextField(_editor.GUICountryNewDomain);
                GUI.enabled = _editor.countryIndex >= 0;
                if (GUILayout.Button("Update")) {
                    _editor.CountryChangeDomain(); //Country Change domain
                }
                if (GUILayout.Button("Rename")) {
                    if (EditorUtility.DisplayDialog("Continent Renaming", "This option will rename the continent affecting to all countries in same continent. Continue?", "Yes", "No")) {
                        _editor.CountryDomainRename(); //Country Re - domain
                    }
                }
                EditorGUILayout.EndHorizontal();
            ...
            if (_editor.countryIndex >= 0) {
                EditorGUILayout.Separator();
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("Event", GUILayout.Width(90));
                int categorySelection = EditorGUILayout.Popup(_editor.GUICategoryIndex, categoryNames);
                if (categorySelection != _editor.GUICategoryIndex) {
                    _editor.CategorySelectByCombo(categorySelection);
                    if (_editor.categoryIndex >= 0)
                        FocusSpherePoint(_map.categories[_editor.categoryIndex].localPosition);
                }
                EditorGUILayout.EndHorizontal();
                if (_editor.categoryIndex >= 0) {
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Name", GUILayout.Width(90));
                    _editor.GUICategoryNewName = EditorGUILayout.TextField(_editor.GUICategoryNewName);
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Class", GUILayout.Width(90));
                    _editor.GUICategoryClass = (CATEGORY_CLASS)EditorGUILayout.IntPopup((int)_editor.GUICategoryClass, categoryClassOptions, categoryClassValues);
                    EditorGUILayout.EndHorizontal();
                    
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Year", GUILayout.Width(90));
                    _editor.GUICategoryYear = EditorGUILayout.TextField(_editor.GUICategoryYear);
                    EditorGUILayout.EndHorizontal();
                    

                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Description", GUILayout.Width(90));
                    _editor.GUICategoryNewDescription = EditorGUILayout.TextField(_editor.GUICategoryNewDescription);
                    EditorGUILayout.EndHorizontal();
                    
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Link ", GUILayout.Width(90));
                    _editor.GUICategoryNewLink = EditorGUILayout.TextField(_editor.GUICategoryNewLink);
                    EditorGUILayout.EndHorizontal();

                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("   Lat/Lon", GUILayout.Width(90));
                    _editor.GUICategoryLatLon = EditorGUILayout.Vector2Field("", _editor.GUICategoryLatLon);
                    EditorGUILayout.EndHorizontal();

                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label("");
                    if (GUILayout.Button("Update Event")) {
                        UndoPushCategoryStartOperation("Undo Event Change");
                        _editor.CategoryUpdate();
                        UndoPushCategoryEndOperation();
                    }
                    EditorGUILayout.EndHorizontal();


                    // Category attributes, da modificare per avere dei tag in più?
                    Category category = _map.categories[_editor.categoryIndex];
                    if (categoryAttribGroup == null) {
                        categoryAttribGroup = new EditorAttribGroup();
                    }
                    if (categoryAttribGroup.itemGroup != category) {
                        categoryAttribGroup.SetItemGroup(category);
                    }
                    if (ShowAttributeGroup(categoryAttribGroup, "Event Attributes")) {
                        _editor.categoryAttribChanges = true;
                        GUIUtility.ExitGUI();
                        return;
                    }
        ...
        void ShowReshapingCategoryTools() {
            GUILayout.BeginVertical();

            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            DrawWarningLabel("CATEGORY MODIFYING TOOLS");
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            RESHAPE_CATEGORY_TOOL prevTool = _editor.reshapeCategoryMode;
            int selectionGridRows = (reshapeCategoryToolbarIcons.Length - 1) / 2 + 1;
            GUIStyle selectionGridStyle = new GUIStyle(GUI.skin.button);
            selectionGridStyle.margin = new RectOffset(2, 2, 2, 2);
            _editor.reshapeCategoryMode = (RESHAPE_CATEGORY_TOOL)GUILayout.SelectionGrid((int)_editor.reshapeCategoryMode, reshapeCategoryToolbarIcons, 2, selectionGridStyle, GUILayout.Height(24 * selectionGridRows), GUILayout.MaxWidth(150));
            if (_editor.reshapeCategoryMode != prevTool) {
                ProcessOperationMode();
            }
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            ...
            GUIStyle explanationStyle = new GUIStyle(GUI.skin.box);
            explanationStyle.normal.textColor = EditorGUIUtility.isProSkin ? new Color(0.52f, 0.66f, 0.9f) : new Color(0.32f, 0.33f, 0.6f);
            GUILayout.Box(reshapeCategoryModeExplanation[(int)_editor.reshapeCategoryMode], explanationStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
            EditorGUILayout.EndHorizontal();

            switch (_editor.reshapeCategoryMode) {
                case RESHAPE_CATEGORY_TOOL.DELETE:
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    if (_editor.categoryIndex < 0) {
                        DrawWarningLabel("Select a category to delete.");
                    } else {
                        DrawWarningLabel("Confirm delete this category?");
                        if (GUILayout.Button("Delete", GUILayout.Width(80))) {
                            UndoPushCategoryStartOperation("Undo Delete Category");
                            _editor.DeleteCategory();
                            UndoPushCategoryEndOperation();
                            _editor.SetInfoMsg(INFO_MSG_CATEGORY_DELETED);
                            _editor.operationMode = OPERATION_MODE.SELECTION;
                        }
                        if (GUILayout.Button("Cancel", GUILayout.Width(80))) {
                            _editor.reshapeCategoryMode = RESHAPE_CATEGORY_TOOL.MOVE;
                        }
                    }
                    GUILayout.FlexibleSpace();
                    EditorGUILayout.EndHorizontal();
...
#region Processing logic
        ...
        static void RestoreBackup(MenuCommand command) {
            ...
            // Categories10
            AssetDatabase.DeleteAsset(geoDataFolder + "/categories10.txt");
            AssetDatabase.SaveAssets();
            AssetDatabase.CopyAsset(backupFolder + "/categories10.txt", geoDataFolder + "/categories10.txt");
            ...
            switch (_editor.operationMode) {
                case OPERATION_MODE.SELECTION:
                    ...
                    if (mouseDown) {
                        ...
                        if (!_editor.CategorySelectByScreenClick(ray, _editor.countryIndex)) {
                            _editor.ClearCategorySelection();
                        }
                ...
                case OPERATION_MODE.RESHAPE:
                    ...
                    switch (_editor.reshapeCategoryMode) {
                        case RESHAPE_CATEGORY_TOOL.MOVE:
                            ExecuteCategoryMoveTool();
                            break;
                    }
                    switch (_editor.reshapeMountPointMode) {
                ...
                case OPERATION_MODE.CREATE:
                    switch (_editor.createMode) {
                        ...
                        case CREATE_TOOL.PROVINCE_REGION:
                            ...
                            break;
                        case CREATE_TOOL.CATEGORY:
                            ExecuteCategoryCreateTool(e.mousePosition,mouseDown);
                            break;
        ...
        //MODIFICA
        void ExecuteCategoryCreateTool(Vector2 mousePosition, bool clicked) {
            ...
            if (_map.GetGlobeIntersection(ray, out cursorPos)) {
                ...
                if (clicked) {
                    Debug.Log("Clicked Execute Create Category");
                    if (_editor.countryIndex < 0 || _editor.countryIndex >= _map.countries.Length) {
                        EditorUtility.DisplayDialog("Add new event", "Please choose a country first.", "Ok");
                        return;
                    }
                    UndoPushCategoryStartOperation("Undo Create Category");
                    _editor.CategoryCreate(_editor.cursor);
                    UndoPushCategoryEndOperation();
                    _editor.operationMode = OPERATION_MODE.SELECTION;
                    return;
                }
        ...
        void AdjustCategoryIconsScale() {
            // Adjust category icons in scene view
            if (_map == null || _map.categories == null)
                return;

            Transform t = _map.transform.Find("Categories");
            if (t != null) {
                Camera cam = GetSceneViewCamera();
                float f = cam != null ? ((cam.transform.position - _map.transform.position) / _map.transform.localScale.x).sqrMagnitude : 1.0f;
                CategoryScaler scaler = t.GetComponent<CategoryScaler>();
                if (scaler != null)
                    scaler.ScaleCategories(0.005f * f);
            } else {
                // This should not happen but maybe the user deleted the layer. Forces refresh.
                _map.showCategories = true;
                _map.DrawCategories(-1);
            }
        }
        
       /* void AdjustCategoryIconsScaleFilter(int filterClass) {
            // Adjust category icons in scene view
            if (_map == null || _map.categories == null)
                return;
            ...
            if (t != null) {
                ...
                // This should not happen but maybe the user deleted the layer. Forces refresh.
                _map.showCategories = true;
                _map.DrawFilterCategories(filterClass);
                
                _map.DrawCategories();
            }
        }*/

        void ShowCategorySelected() {
            if (_editor.categoryIndex < 0 || _editor.categoryIndex >= _map.categories.Count)
                return;
            Vector3 categoryPos = _map.categories[_editor.categoryIndex].localPosition;
            Vector3 worldPos = _map.transform.TransformPoint(categoryPos);
            float handleSize = HandleUtility.GetHandleSize(worldPos) * HANDLE_SIZE * 2.0f;
            Handles.RectangleHandleCap(0, worldPos, frontFaceQuaternion, handleSize, EventType.Repaint);
        }

        void ExecuteCategoryMoveTool() {
            if (_editor.categoryIndex < 0 || _editor.categoryIndex >= _map.categories.Count)
                return;

            Transform mapTransform = _map.transform;
            Vector3 categoryPos = _map.categories[_editor.categoryIndex].localPosition;
            Vector3 oldPoint = mapTransform.TransformPoint(categoryPos);
            float handleSize = HandleUtility.GetHandleSize(oldPoint) * HANDLE_SIZE * 2.0f;
            Vector3 newPoint = Handles.FreeMoveHandle(oldPoint, frontFaceQuaternion, handleSize, pointSnap,
                                            (handleControlID, position, rotation, size, eventType) => {
                                                Handles.RectangleHandleCap(handleControlID, position, rotation, size, eventType);
            ...
            if (newPoint != oldPoint) {
                newPoint = mapTransform.InverseTransformPoint(newPoint);
                if (!startedReshapeCity)
                    UndoPushCityStartOperation("Undo Category Move");
                _editor.CategoryMove(newPoint);
                HandleUtility.Repaint();
            }
        }
        //
        ...
        //MODIFICA
        void UndoPushCategoryStartOperation(string operationName) {
            startedReshapeCategory = !startedReshapeCategory;
            undoPushStarted = true;
            Undo.RecordObject(target, operationName);   // record changes to the undo dummy flag
            _editor.UndoCategoriesPush();
        }

        void UndoPushCategoryEndOperation() {
            undoPushStarted = false;
            _editor.UndoCategoriesInsertAtCurrentPos();
        }
        ...
        static void CheckBackup(out string geoDataFolder) {
            ...
            if (!backupFolderExists) {
                string categoryFilename = "categories" + "_" + activeScene + "_ENGLISH.txt";
                string countries10Filename = "countries10" + "_" + activeScene + "_ENGLISH.txt";
                string countries110Filename = "countries110" + "_" + activeScene + "_ENGLISH.txt";
                string citiesFilename = "cities" + "_" + activeScene + "_ENGLISH.txt";
                string provincesFilename = "provinces" + "_" + activeScene + "_ENGLISH.txt";
                string mountPointsFilename = "mountPoints" + "_" + activeScene + "_ENGLISH.txt";
                // Do the backup
                AssetDatabase.CreateFolder(rootFolder, "Backup");
                string backupFolder = rootFolder + "/Backup";
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Countries" + countries10Filename, backupFolder + countries10Filename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Countries" + countries110Filename, backupFolder + countries110Filename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Countries" + globe.countryAttributeFile, backupFolder + "/" + globe.countryAttributeFile +"_Backup");
                ...
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Provinces" + provincesFilename, backupFolder + provincesFilename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Provinces" + globe.provinceAttributeFile, backupFolder + "/" + globe.provinceAttributeFile +"_Backup");
                
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Cities" + citiesFilename, backupFolder + citiesFilename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Cities" + globe.cityAttributeFile, backupFolder + "/" + globe.cityAttributeFile +"_Backup");
                ...
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/MountPoints" + mountPointsFilename, backupFolder + mountPointsFilename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/MountPoints" + globe.mountPointsAttributeFile, backupFolder + "/" + globe.mountPointsAttributeFile +"_Backup");

                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Categories" + categoryFilename, backupFolder + categoryFilename + "_Backup");
                AssetDatabase.CopyAsset(geoDataFolder + "/" + activeScene + "/Provinces" + globe.categoryAttributeFile, backupFolder + "/" + globe.categoryAttributeFile +"_Backup");
                
                //AGGIUNGERE CATEGORIEEE
            }
        }
        ...
        bool SaveChanges() {
            if (!_editor.countryChanges && !_editor.countryAttribChanges && !_editor.provinceChanges && !_editor.provinceAttribChanges && !_editor.cityChanges && !_editor.cityAttribChanges && !_editor.mountPointChanges  && !_editor.categoryChanges)
                return false;
            ...
            if (_editor.countryAttribChanges || _editor.countryChanges) {
                fullPathName = GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Countries" + "/" +  _map.countryAttributeFile + ".json";;
            ...
            if (_editor.countryChanges) {
                ...
                fullPathName = GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Countries" + "/" + dataFileName;
            ...
            if (_editor.provinceAttribChanges || _editor.provinceChanges) {
                fullPathName = GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Provinces" + "/" +  _map.provinceAttributeFile + ".json";
            ...
            if (_editor.provinceChanges) {
                ...
                fullPathName =  GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Provinces" + "/" + dataFileName;
            ...
            if (_editor.cityAttribChanges || _editor.cityChanges) {
                fullPathName =  GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Cities" + "/" +  _map.cityAttributeFile + ".json";
            ...
            if (_editor.cityChanges) {
                ...
                fullPathName =  GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Cities" + "/" +  dataFileName;
            ...
            // Save changes to categories attributes
            if (_editor.categoryAttribChanges || _editor.categoryChanges) {
                fullPathName = GetAssetsFolder() + geoDataFolder + "/" + _map.categoryAttributeFile + ".json";
                string data = _map.GetCategoriesAttributes(true);
                if (data != null) {
                    File.WriteAllText(fullPathName, data, System.Text.Encoding.UTF8);
                }
                _editor.categoryAttribChanges = false;
            }
            // Save changes to categories
            if (_editor.categoryChanges) {
                dataFileName = _editor.map.GetCategoryGeoDataFileName();
                fullPathName = GetAssetsFolder() + geoDataFolder + "/" + SceneManager.GetActiveScene().name + "/" + "Categories" + "/" + dataFileName;
                string data = _editor.map.GetCategoryGeoData();
                if (data != null) {
                    File.WriteAllText(fullPathName, data, Encoding.UTF8);
                    Debug.Log("Category geodata file updated.");
                }
                _editor.categoryChanges = false;
            }

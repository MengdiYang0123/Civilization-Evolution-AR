...
namespace WPM {
	public partial class WorldMapEditor : MonoBehaviour {
		...
		public int GUIDomainIndex;
		public string GUICountryName = "";
		public string GUICountryNewName = "";
		public string GUICountryDomain = "";
		public string GUICountryNewDomain = "";
		...
		public bool CountryDomainRename () {

			if (countryIndex < 0)
				return false;

			string currentDomain = map.countries [countryIndex].domain;
			if (currentDomain != GUICountryNewDomain)
				map.countries[countryIndex].domain = GUICountryNewDomain;
			
			countryChanges = true;
			return true;
		}

		public bool CountryChangeDomain () {
			if (countryIndex < 0 || countryIndex >= map.countries.Length)
				return false;
			map.countries [countryIndex].domain = GUICountryNewDomain;
			GUICountryDomain = GUICountryNewDomain;
			countryChanges = true;
			return true;
		}
		...
		public void CountrySelectByDomain (int selection) {
			GUICountryName = "";
			GUIDomainIndex = selection;
			if (GetCountryIndexByGUISelection ()) {
				if (Application.isPlaying) {
					map.BlinkCountry (countryIndex, Color.black, Color.green, 1.2f, 0.2f);
				}
		...
		public void CountryRegionSelect () {
			...
			GUICountryNewDomain = map.countries[countryIndex].domain;
			...
			if (GUICountryName.Length > 0) {
				for (int k = 0; k < _countryNames.Length; k++) {  // don't use countryNames or the array will be reloaded again if grouped option is enabled causing an infinite loop
					if (_countryNames [k].TrimStart ().StartsWith (GUICountryName)) {
						...
						GUICountryDomain = map.countries[countryIndex].domain;
						return;
					}
				}
		...
		public void CountryCreate () {
			...
			Country newCountry = new Country ("New Country" + (countryIndex + 1).ToString (), continent ,  "New Domain");
		...
		public void CountryCreate(Region region) {
			...
			Country newCountry = new Country(uniqueName, country.continent , country.domain);
			if (entity is Country) {
				newCountry.regions.Add(region);
			} else {

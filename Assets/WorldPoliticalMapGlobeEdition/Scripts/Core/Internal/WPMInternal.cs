...
namespace WPM
{
    ...
    public partial class WorldMapGlobe : MonoBehaviour
    {
        ...
        public void Init()
        {
            ...
            categorySpotDiscovery = Resources.Load<GameObject>("Prefabs/CategorySpotDiscovery");
            categorySpotScience = Resources.Load<GameObject>("Prefabs/CategorySpotScience");
            categorySpotTreaty = Resources.Load<GameObject>("Prefabs/CategorySpotTreaty");
            categorySpotFunFact = Resources.Load<GameObject>("Prefabs/CategorySpotFunFact");
            categorySpotHumanRights = Resources.Load<GameObject>("Prefabs/CategorySpotHumanRights");
            categorySpotTragedy = Resources.Load<GameObject>("Prefabs/CategorySpotTragedy");
            ...
            //BATTLE MATERIAL
            categoriesBattleMat = Instantiate(Resources.Load<Material>("Materials/CategoryBattle"));
            categoriesBattleMat.name = "CategoriesBattle";
            //ART MATERIAL
            categoriesArtMat = Instantiate(Resources.Load<Material>("Materials/CategoryArt"));
            categoriesArtMat.name = "CategoriesArt";
            //DISCOVERY MATERIAL
            categoriesDiscoveryMat = Instantiate(Resources.Load<Material>("Materials/CategoryDiscovery"));
            categoriesDiscoveryMat.name = "CategoriesDiscovery";
            //SCIENCE MATERIAL
            categoriesScienceMat = Instantiate(Resources.Load<Material>("Materials/CategoryScience"));
            categoriesScienceMat.name = "CategoriesScience";
            //TREATY MATERIAL
            categoriesTreatyMat = Instantiate(Resources.Load<Material>("Materials/CategoryTreaty"));
            categoriesTreatyMat.name = "CategoriesTreaty";
            //FUN FACT MATERIAL
            categoriesFunFactMat = Instantiate(Resources.Load<Material>("Materials/CategoryFunFact"));
            categoriesFunFactMat.name = "CategoriesFunFact";
            //HUMAN RIGHTS MATERIAL
            categoriesHumanRightsMat = Instantiate(Resources.Load<Material>("Materials/CategoryHumanRights"));
            categoriesHumanRightsMat.name = "CategoriesHumanRights";
            //TRAGEDY MATERIAL
            categoriesTragedyMat = Instantiate(Resources.Load<Material>("Materials/CategoryTragedy"));
            categoriesTragedyMat.name = "CategoriesTragedy";
            //EVENT MATERIAL
            categoriesEventMat = Instantiate(Resources.Load<Material>("Materials/CategoryEvent"));
        ...
        /// </summary>
        public void ReloadData(LANGUAGES_OPTIONS lingua)
        {

            //Di base la lingua è in inglese
            //Debug.Log("Passo 0: Lettura lingua.");

            //ChangeLanguage(1); //Carico la mappa in italiano 

            //Debug.Log("Passo 1: Caricamento Scena con lingua corretta  ");
            // read baked data
            ...
            if (_showCities)
            {
                ReadCitiesPackedString(SceneManager.GetActiveScene().name, lingua);
            }
            else
            {
                _cities = null;
            ...
            if (_showCategories)
            {
                //Passo scena = periodo + lingua --> Switch case in base alla lingua
                ReadCategoriesPackedString(SceneManager.GetActiveScene().name, lingua);
            }
            else
            {
                _categories = null;
...
#if VR_GOOGLE || VR_SAMSUNG_GEAR_CONTROLLER || VR_OCULUS || VR_HOLOLENS
            _VREnabled = true;
#endif
        ...
        void CheckEventsOverMap()
        {
            ...
            if (mouseIsOver || _VREnabled)
            {
                ...
                if (isClick || isRelease)
                {
                    ...
                    _categoryLastClicked = _categoryHighlightedIndex;
                    if (_categoryLastClicked >= 0)
                    {
                        if (isClick && !fullClick && OnCategoryPointerDown != null)
                        {
                            OnCategoryPointerDown(_categoryLastClicked);
                        }
                        else if (isRelease)
                        {
                            if (OnCategoryPointerUp != null)
                            {
                                OnCategoryPointerUp(_categoryLastClicked);
                            }
                            if (fullClick && OnCategoryClick != null)
                            {
                                //Si potrebbe aggiungere una funzione faccia apparire un POP-UP, funzione da aggiungere nella "DEMO"
                                OnCategoryClick(_categoryLastClicked);
                            }
                        }
                    }
        ...
        public void Redraw(bool rebuildFrontiers = false)
        {
            ...
            DrawCategories(_filterCategoriesClass);  //Redraw Categories layer  //MODIFICA
        ...
        bool TryGetHandRay(out Ray ray)
        {
            ray = new Ray();
            ...
            foreach (var source in CoreServices.InputSystem.DetectedInputSources)
            {
                if (source.SourceType == InputSourceType.Hand)
                {
                    ...
                    {
                        if (pointer is IMixedRealityPointer mixedRealityPointer && mixedRealityPointer.Result != null)
                        {
                            ray = new Ray(mixedRealityPointer.Position, mixedRealityPointer.Rotation * Vector3.forward);
                            return true;
                        }
                    }
                }
            ...
            if (UpdateCursorLocation())
            {
                ...
                if (GetCountryUnderMouse(cursorLocation, out c, out cr))
                {
                    ...
                    if (!ignoreCountryByEvent)
                    {
                        ...
                        {
                            int cat;
                            if (GetCategoryUnderMouse(c, cursorLocation, out cat))
                            {
                                if (cat != _categoryHighlightedIndex)
                                {
                            ...
                            else if (_categoryHighlightedIndex >= 0)
                            {
                                HideCategoryHighlight();
                            }
                        }
